...
export default function PlayPage() {
  ...
  if (!game) {
    return (
      <div className="min-h-screen game-table p-4 sm:p-8">
        <div className="max-w-lg mx-auto">
          <h1 className="text-2xl sm:text-3xl font-bold mb-2 text-gray-900">The Descent</h1>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6 shadow-sm">
              ...
              {playerNames.map((name, i) => (
                <div key={i} className="flex gap-2 items-center">
                  <input
                    ...
                    className="flex-1 bg-gray-50 border border-gray-300 rounded px-3 py-2 text-gray-900"
            ...
            {playerNames.length < 6 && (
              <button
                ...
                className="w-full py-2 mb-4 bg-gray-50 rounded hover:bg-gray-100 border border-gray-300 text-gray-600"
              >
                + Add Player
              </button>
            )}
          ...
          <button
            onClick={startGame}
            className="w-full py-3 bg-emerald-600 text-white rounded-lg font-bold text-lg hover:bg-emerald-500 transition-colors"
          >
            Start Game
          </button>
        </div>
    ...
    return (
      <div className="min-h-screen game-table p-4 sm:p-8">
        <div className="max-w-2xl mx-auto">
          <h1 className="text-3xl sm:text-4xl font-bold mb-2 text-center text-gray-900">Game Over!</h1>
          <p className="text-center text-emerald-600 text-lg sm:text-xl mb-6 sm:mb-8">
            Winner: {standings[0].name}
          </p>

          <div className="bg-white rounded-xl border border-gray-200 p-3 sm:p-4 mb-6 overflow-x-auto shadow-sm">
            <h2 className="font-bold mb-3 text-gray-900">Final Standings</h2>
            <table className="w-full text-xs sm:text-sm text-gray-700">
              ...
              <tbody>
                {standings.map(s => (
                  <tr key={s.name} className={s.rank === 1 ? 'text-emerald-600 font-bold' : ''}>
          ...
          <button
            ...
            className="mt-4 w-full py-3 bg-emerald-600 text-white rounded-lg font-bold hover:bg-emerald-500"
          >
            New Game
          </button>
        </div>
  ...
  // ── Tabletop Layout: Players around the table, trail cards in center ──
  // Arrange player indices around the table: top, right, bottom, left
  ...
  return (
    <div className="min-h-screen game-table overflow-auto">
      ...
      <div className="bg-white border-b border-gray-200 px-4 py-2 sticky top-0 z-10">
        <div className="flex items-center justify-between flex-wrap gap-2">
          <div className="flex items-center gap-3">
            <h1 className="text-base sm:text-lg font-bold text-gray-900">The Descent</h1>
          ...
          <div className="flex gap-2 items-center">
            {game.phase !== 'sprint' && (game.phase as string) !== 'game_over' && (
              <button onClick={doAdvance} className="px-3 py-1.5 bg-emerald-600 text-white rounded font-bold text-xs hover:bg-emerald-500 transition-colors">
                Next Phase &rarr;
              </button>
            )}
      ...
      <div className="p-3 sm:p-4">
        {/* ═══ TABLE LAYOUT: Players around edges, trail cards in center ═══ */}
        <div className="flex flex-col gap-3 mb-4">
          ...
          {/* CENTER: Trail Cards + Obstacles + Actions */}
          <div className="flex gap-4 items-start justify-center flex-wrap">
            {/* Trail Cards — the center of the table */}
            <div className="flex flex-col items-center gap-2">
              <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider">Trail</h3>
              <div className="flex gap-3 items-start">
                ...
                <div className="flex flex-col items-center gap-1">
                  ...
                  <div className="card-back deck-pile flex items-center justify-center" style={{ width: '80px', height: '115px' }}>
                    <div className="text-white/80 text-xs font-bold bg-black/40 rounded px-2 py-1">{game.trailDeck.length}</div>
                  </div>
                </div>
              </div>

              {/* Standings below trail cards */}
              <div className="trail-card p-3 min-w-[200px] mt-1">
                ...
                {standings.map(s => (
                  <div key={s.name} className="flex justify-between text-xs py-0.5 gap-3">
                    ...
                    <span className="font-mono">
                      <span className="text-cyan-600">{s.obstaclesCleared}</span>
                  ...
                  return (
                    <div key={i} className="obstacle-card flex flex-col items-center justify-center" style={{ width: '130px', padding: '8px' }}>
                      <div className="flex gap-1 mb-1">
                        {obs.symbols.map((sym, j) => (<span key={j} className="text-2xl">{SYMBOL_EMOJI[sym]}</span>))}
                      </div>
                        ...
                        {UPGRADES.map(upgrade => {
                          ...
                          return (
                            <button key={upgrade.id}
                              ...
                              <div className="font-bold">{upgrade.name} <span className="text-amber-500">({upgrade.flowCost}F)</span></div>
        ...
        {/* ═══ Effect Toast ═══ */}
        {effectToast && (
          <div className="animate-pulse rounded-lg px-4 py-3 mb-2 border-2 flex items-center gap-3"
            style={{ borderColor: effectToast.color, backgroundColor: `${effectToast.color}10`, boxShadow: `0 0 20px ${effectToast.color}20` }}>
            <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: effectToast.color }} />
            <div>
              <span className="font-bold text-sm" style={{ color: effectToast.color }}>{effectToast.cardName}</span>
              <span className="text-gray-600 text-sm ml-2">{effectToast.text}</span>
            </div>
          </div>
        )}
        ...
        <div className="flex flex-wrap gap-4">
          <div className="flex-1 min-w-[300px]">
            <h3 className="text-xs font-bold mb-2 text-gray-400 uppercase tracking-wider">
              ...
              {game.phase === 'sprint' && <span className="text-gray-500 normal-case"> (Click to play = 1 Action)</span>}
            </h3>
            <HandDisplay
          ...
          {game.players.some(p => p.penalties.length > 0) && (
            <div className="min-w-[180px]">
              <h3 className="text-xs font-bold mb-2 text-orange-500 uppercase tracking-wider">Penalty Cards</h3>
              {game.players.map(player => {
                if (player.penalties.length === 0) return null;
                return (
                  <div key={player.id} className="mb-2">
                    <div className="text-[10px] text-gray-500 font-bold mb-1">{player.name} ({player.penalties.length})</div>
                    <div className="flex flex-wrap gap-1.5">
                      {player.penalties.map((pen, i) => (
                        <div key={i} className="rounded-lg px-2.5 py-2 text-xs bg-orange-50 border-2 border-orange-300">
                          <div className="font-bold text-orange-700">{pen.name}</div>
                          <div className="text-orange-500 text-[10px] mt-0.5">{pen.description}</div>
                        </div>
                      ))}
                    </div>
                  </div>
...
function FlowButton({
  ...
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className="text-left p-2 rounded-lg bg-purple-50 hover:bg-purple-100 border border-purple-200 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
    >
      <div className="flex items-center justify-between mb-0.5">
        <span className="font-bold text-xs text-purple-700">{label}</span>
        <span className="text-[10px] font-mono text-purple-500">{cost}F</span>
      </div>
      <div className="text-[9px] text-purple-400 leading-tight">{description}</div>
    </button>
  );
}
