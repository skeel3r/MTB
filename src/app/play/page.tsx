    ...
    return (
      <div className="min-h-screen bg-gray-950 text-white p-8">
        <div className="max-w-lg mx-auto">
          <h1 className="text-3xl font-bold mb-2">The Descent</h1>
              ...
              <tbody>
                {standings.map(s => (
                  <tr key={s.name} className={s.rank === 1 ? 'text-emerald-400 font-bold' : ''}>
                    ...
                    <td className="text-center">{s.perfectMatches}</td>
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="bg-gray-900 border-b border-gray-700 px-4 py-3 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold">The Descent</h1>
        ...
        <div className="flex gap-2">
          {game.activeTrailCard && (
            <div className="bg-gray-800 px-3 py-1 rounded text-sm">
              ...
              <span className="text-yellow-400">(Limit: {game.activeTrailCard.speedLimit})</span>
            </div>
          )}
      ...
      <div className="flex">
        {/* Left panel: Player selector + Board */}
        ...
        <div className="flex-1 p-4">
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="px-4 py-3 bg-gray-700 rounded-lg hover:bg-gray-600 border border-gray-500"
                >
                  <div className="font-bold">Main Line</div>
                  <div className="text-xs text-gray-400">Obstacles = +1 Progress</div>
                </button>
                <button
                  onClick={() => doAction({ type: 'commit_line', payload: { line: 'pro' } })}
                  className="px-4 py-3 bg-red-900/50 rounded-lg hover:bg-red-900 border border-red-700"
                >
                  <div className="font-bold text-red-400">Pro Line</div>
                  <div className="text-xs text-gray-400">Obstacles = +2 Progress, Cannot Brake</div>
                </button>
              </div>
            </div>
          )}
                  ...
                  {[0, 1, 2, 3, 4, 5].map(r => (
                    <div key={r} className="flex gap-1">
                      <button
                        ...
                        className="px-2 py-1 bg-gray-700 rounded hover:bg-gray-600 disabled:opacity-30"
                      >
                        R{r + 1} &larr;
                      </button>
                      <button
          ...
          {game.phase !== 'sprint' && (game.phase as string) !== 'game_over' && (
            <button
              onClick={doAdvance}
              className="mt-4 px-6 py-3 bg-emerald-600 rounded-lg font-bold hover:bg-emerald-500 transition-colors"
            >
              Next Phase &rarr;
            </button>
          )}
...
function ActionButton({
  ...
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className={`px-4 py-2 rounded-lg font-bold text-sm ${color} disabled:opacity-30 disabled:cursor-not-allowed transition-colors`}
    >
      {label}
    </button>
  );
